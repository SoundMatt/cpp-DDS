...
| `include/dds/channel.hpp` | `relay::Channel<T>` — bounded, thread-safe FIFO channel (`dds::Chan<T>` alias) |
...
| `tara.md` / `tara.json` | ISO/SAE 21434 threat analysis, generated by `cpfusa tara` (§20.4) |
| `fmea.csv` / `fmea.json` | Generated dFMEA, `cpfusa fmea` (§20.4) |
...
- `relay::Channel<T>` (aliased as `dds::Chan<T>`) is non-copyable, non-movable; always heap-allocate via `shared_ptr`.
