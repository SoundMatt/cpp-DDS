...
//
// cli/cpp_dds_cli.cpp — implementation of cpp-dds's subcommand dispatch and
// each subcommand. See cpp_dds_cli.hpp for why this is a separate
// translation unit from main.cpp (mirrors ddstool/cli.cpp's own
// precedent) and why it isn't named cli.cpp.
//
...
const     char*       kSpecVersion = dds::kSpecVersion;
...
// format_flag scans args for "--format <value>" and returns its value, or
// "text" if absent (or if "--format" is the last argument, with no value
// following it).
std::string format_flag(const std::vector<std::string>& args) {
...
dds::Sample parse_dds_sample(const json::Value& v) {
    if (!v.is_object()) throw json::ParseError("dds.Sample: expected a JSON object");

    dds::Sample s;

    const auto* topic = v.find("topic");
    if (!topic) throw json::ParseError("dds.Sample: missing required field \"topic\"");
    s.topic = topic->as_string();

    const auto* payload = v.find("payload");
    if (!payload) throw json::ParseError("dds.Sample: missing required field \"payload\"");
    auto decoded = base64::decode(payload->as_string());
    if (!decoded) throw json::ParseError("dds.Sample: \"payload\" is not valid base64");
    s.payload = std::move(*decoded);

    const auto* timestamp = v.find("timestamp");
    if (!timestamp) throw json::ParseError("dds.Sample: missing required field \"timestamp\"");
    if (!looks_like_rfc3339(timestamp->as_string()))
        throw json::ParseError("dds.Sample: \"timestamp\" is not a valid RFC 3339 date-time");
    s.timestamp = std::chrono::system_clock::time_point{}; // zeroed — see cmd_convert.

    const auto* seq = v.find("seq");
    if (!seq) throw json::ParseError("dds.Sample: missing required field \"seq\"");
    double seq_n = seq->as_number();
    if (seq_n < 0) throw json::ParseError("dds.Sample: \"seq\" must not be negative");
    s.sequence_number = static_cast<uint64_t>(seq_n);

    const auto* guid = v.find("writer_guid");
    if (!guid) throw json::ParseError("dds.Sample: missing required field \"writer_guid\"");
    const auto& items = guid->as_array();
    if (items.size() != 16)
        throw json::ParseError("dds.Sample: \"writer_guid\" must have exactly 16 elements");
    for (std::size_t i = 0; i < 16; ++i) {
        double b = items[i].as_number();
        if (b < 0 || b > 255)
            throw json::ParseError("dds.Sample: \"writer_guid\" elements must be 0-255");
...
void write_relay_message_json(std::ostream& out, const relay::Message& m) {
    out << "{\n"
        ...
        << "    },\n"
        << "    \"id\": " << json::escape(m.id) << ",\n"
        << "    \"payload\": " << json::escape(base64::encode(m.payload)) << ",\n"
...
void print_usage(std::ostream& out) {
    out << "Usage: cpp-dds <version|capabilities|status|conform|convert> [args...]\n"
...
int dispatch(const std::vector<std::string>& args, std::istream& in,
    ...
    if (cmd == "version")      return cmd_version(format_flag(rest), out);
    if (cmd == "capabilities") return cmd_capabilities(out);
    if (cmd == "status")       return cmd_status(format_flag(rest), out);
    if (cmd == "conform")      return cmd_conform(out, err);
    if (cmd == "convert")      return cmd_convert(rest, in, out, err);
